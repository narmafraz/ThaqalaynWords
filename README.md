...
| `morphology.lex` | CAMeL Tools' canonical lemma identifier. **Undiacritized.** Used internally to drive the paradigm generator. |
...
```json
{
  ...
  "paradigm": [
    ...
    {"role": "present_3ms", "form": "يَقُول", "diacritized": "يَقُول",
...
| `paradigm[].diacritized` | Same as `form` here — kept distinct so a future normalization change can adjust one without the other. |
